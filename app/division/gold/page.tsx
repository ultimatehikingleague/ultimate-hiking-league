...
async function fetchAllGoldHikers() {
  const pageSize = 1000
  let from = 0
  let allRows: any[] = []
  ...
  if (seasonError) {
    console.error('Gold season stats load error:', seasonError)
    return []
  }
...
export default function GoldPage() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        const hikers = await fetchAllGoldHikers()
        ...
        console.error('Gold ranking load error:', error)
      } finally {
        setLoading(false)
      }
    }
      ...
      <section className="px-6 py-10 md:px-10 md:py-12">
        <div className="mx-auto max-w-5xl">
          <div className="mb-5 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            <div>
              <h2 className="text-2xl font-bold text-white">Gold Ranking</h2>
