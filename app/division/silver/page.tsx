...
async function fetchAllSilverHikers() {
  const pageSize = 1000
  let from = 0
  let allRows: any[] = []
  ...
  if (seasonError) {
    console.error('Silver season stats load error:', seasonError)
    return []
  }
...
export default function SilverPage() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        const hikers = await fetchAllSilverHikers()
        ...
        console.error('Silver ranking load error:', error)
      } finally {
        setLoading(false)
      }
    }
      ...
      <section className="px-6 py-10 md:px-10 md:py-12">
        <div className="mx-auto max-w-5xl">
          <div className="mb-5 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            <div>
              <h2 className="text-2xl font-bold text-white">Silver Ranking</h2>
              ...
              paginatedUsers.map((user) => (
                ...
                >
                  ...
                  <div className="text-right">
                    ...
                    <div className="text-xs text-stone-400">silver</div>
                  </div>
                </div>
              ))
            )}
